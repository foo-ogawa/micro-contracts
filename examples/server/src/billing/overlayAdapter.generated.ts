...
import type { OverlayResult, OverlayRegistry } from '@project/contract/billing/overlays/index.js';
import type { ProblemDetails } from '@project/contract/billing/schemas/types.js';
...
const extractors: Record<string, ParamExtractor> = {
  ...
  'validatePaymentMethod': (req) => ({
    'X-Payment-Token': getHeader(req, 'X-Payment-Token'),
  }),
};
...
export const endpointOverlays: Record<string, string[]> = {
  'getInvoices': ['requireAuth', 'tenantIsolation'],
  'createInvoice': ['requireAuth', 'tenantIsolation'],
  'getInvoiceById': ['requireAuth', 'tenantIsolation'],
  'payInvoice': ['requireAuth', 'tenantIsolation', 'validatePaymentMethod'],
};
